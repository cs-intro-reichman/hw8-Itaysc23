...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    for (int i = 0; i < userCount; i++) {
        ...
        if (potential.getName().equals(name)) continue;

        if (me.follows(potential.getName())) continue;

        int currentMutuals = me.countMutual(potential);
    ...
    return (bestCandidate != null) ? bestCandidate.getName() : null;
}

    /** Computes and returns the name of the most popular user in this network: 
     *  The user who appears the most in the follow lists of all the users. */
    public String mostPopularUser() {
        String max = this.users[0].getName();
        for ( int i = 0 ; i < this.userCount; i++){
            if (followeeCount(this.users[i].getName())> followeeCount(max)){
